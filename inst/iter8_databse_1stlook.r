...
fig_2 <-
  dat_full %>%
    ggplot(.,aes(x = pct_d1 - pct_t0 , group = dataset)) +
    # histogram with density
    geom_histogram(aes(y = ..density..), bins = 30, fill = "lightgrey", color = "black") +
    # kernel smooth
    geom_density(alpha = 0.5, fill = "blue") +
...
dat_full <- dat_full %>%
  mutate(inf = ifelse(dataset == "Inappropriate",1,
                  ifelse(dataset == "Appropriate",0,NA)),
...
# Print confusion table (NPV=0.47)
confusionMatrix(
...
dat_full$pr_simp <- ifelse(dat_full$pct_t0 >0.5,0,1) %>% factor
...
dat_clean <- dat_full %>%
  ...
  na.omit() %>%
  mutate(inf = as.factor(inf) %>% forcats::fct_recode("Appropriate" = "0",
                                                      "Not_appropriate" = "1"))
...
predictions <- predict(tree_model, dat_clean) %>%
  {ifelse(.[,2] > 0.8, "Not_appropriate", "Appropriate")} %>%
  factor()
